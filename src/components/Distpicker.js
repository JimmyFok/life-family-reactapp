...
const provinces = ['安徽', '澳门', '北京', '福建', '甘肃', '广东', '广西', '贵州', '海南', '河北', '河南', '黑龙江', '湖北', '湖南', '吉林', '江苏', '江西', '辽宁', '内蒙古', '宁夏', '青海', '山东', '山西', '陕西', '上海', '四川', '台湾', '天津', '西藏', '香港', '新疆', '云南', '浙江', '重庆', '其他']
export default class AddReceive extends Component {
    ...
    getPiont(){    
        ...
        geolocation.getCurrentPosition(function(r){
            if(this.getStatus() === 0){
                ...
                that.setState({
                    province: r.address.province.slice(0,-1),
                    city: r.address.city.slice(0,-1),
                    county: that.getCounty(r.address.province.slice(0,-1),r.address.city.slice(0,-1))[0],  // 只能直接取了第一个
                    show: ['dis','dis','blo'],
                    cities: that.getCity(r.address.province.slice(0,-1)),
                    counties: that.getCounty(r.address.province.slice(0,-1),r.address.city.slice(0,-1)),
                })
    ...
    // 因为是在Dom加载后才能获取定位，所以只有在更新state的时候才重新渲染来达到获取当前位置的目的
    shouldComponentUpdate(nextProps, nextState){
        // this.getPiont()
        // 因为show实际上变化，所以在更新的时候通过其元素的变化判断来重新加载
        return nextState.province !== this.state.province || nextState.city !== this.state.city || nextState.county !== this.state.county ||
            nextState.show[0] !== this.state.show[0]||
            nextState.show[1] !== this.state.show[1]||
            nextState.show[2] !== this.state.show[2]
    }

    // 加载前定义好，但是有问题
    componentDidMount(){
       this.getPiont()  // 加载当前的定位     
    ...
    handleChange(name, e) {
        ...
        // 将事件指向dom元素获取其value特性
        let chosed = e.target.getAttribute("value");
        ...
        switch (name) {
            ...
            case "sheng":
                this.setState({
                    province: chosed,  // 保留点击对应的省份
                    city: "",   // 城市
                    county: "",   // 区
                    provinces: provinces,  // 需要重新修改为初始值
                    cities: [],
                    counties: [],
                    show: ['blo','dis','dis']
                });
            ...
            case "shi":
                this.setState({                    
                    ...
                    county: "",   // 区
                    provinces: provinces,  // 需要重新修改为初始值
                    cities: this.getCity(this.state.province),
                    counties: [],
                    show: ['dis','blo','dis']
                });
                break;
            case "qu":
                this.setState({
                    county: chosed,   // 区
                    provinces: provinces,  // 需要重新修改为初始值
                    counties: this.getCounty(this.state.province, this.state.city),
                    show: ['dis','dis','blo']
                });
                break;
            ...
            case "province":
                this.setState({
                    ...
                    city: "",   // 城市
                    county: "",   // 区
                    provinces: [],  // 需要重新修改为初始值
                    cities: this.getCity(chosed),
                    counties: [],
                    show: ['dis','blo','dis']
                });
                break;
            ...
            case "city":            
                this.setState({
                    // province: chosed,  // 保持不变
                    city: chosed,   // 城市
                    county: "",   // 区
                    provinces: [],  // 需要重新修改为初始值
                    cities: [],
                    counties: this.getCounty(this.state.province, chosed),
                    show: ['dis','dis','blo']
                });
                break;
            case "county":
            ...
            <div>                
                <div className="present" style={fixed}>   
                    ...
                    <ul className = "chosed-list">
                        <li onClick={this.handleChange.bind(this, "sheng")} value={this.state.province} className={this.state.province===""?"dis":""}>{this.state.province}省</li>                        
                        <li onClick={this.handleChange.bind(this, "shi")} value={this.state.city} className={this.state.city===""?"dis":""}>{this.state.city}市</li>
                        <li onClick={this.handleChange.bind(this, "qu")} value={this.state.county} className={this.state.county===""?"dis":""}>{this.state.county}区</li>
                    </ul>
                </div>                               
                <div className="city-list" style={related}>     
                    <h4>省市列表</h4>                
                    <ul onClick={this.handleChange.bind(this, "province")} className = {this.state.show[0]}>
                        {this.state.provinces.map(province => (
                            <li value={province} key={id++} className={province===this.state.province?"chosed":""}>{province}</li>
                    ))}
                    </ul>
                    <ul onClick={this.handleChange.bind(this, "city")} className = {this.state.show[1]}>
                        {this.state.cities.map(city => (
                            <li value={city} key={id++} className={city===this.state.city?"chosed":""}>{city}</li>
                        ))}
                    </ul>
                    <ul onClick={this.handleChange.bind(this, "county")} className = {this.state.show[2]}>
                        {this.state.counties.map(county => (
                            <li value={county} key={id++} className={county===this.state.county?"chosed":""}>{county}</li>
                        ))}
                    </ul>
                </div>
            </div>            
